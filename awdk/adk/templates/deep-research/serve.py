...
log = logging.getLogger("deep_research.serve")
...
def resolve_provider() -> tuple[str, str, str | None]:
    ...
    model = os.getenv("DEEP_RESEARCH_MODEL") or None
    explicit = (os.getenv("DEEP_RESEARCH_PROVIDER") or "").strip().lower()
...
def load_pack():
    ...
    pack_dir = ASSETS / "pack" / "deep-research"
...
def build_agent():
    """Construct the AitherAgent with the deep-research pack applied. Returns
    (agent, ledger, session, brain_pack_dict, provider, model). Raises SystemExit
    ...
    from engine.ledger import LedgerRouter, SavingsLedger
    from engine.tools import ResearchSession, build_research_tools

    ledger = SavingsLedger()
    llm = LedgerRouter(provider=provider, api_key=api_key, model=model, ledger=ledger)

    identity = Identity(
        name="researcher",
        description="Senior research analyst — searches, reads sources, cites, writes reports.",
        system_prompt=sys_prompt,
        skills=["deep_research", "report_writing", "fact_checking"],
    )
    # Keep conversation memory INSIDE the deliverable's data dir (adk's default is
    # ~/.aither/memory, which is global) so the app is truly self-contained — delete
    # .data/ to fully reset — and so AITHER_DATA_DIR isolates it for tests.
    (data_dir / "memory").mkdir(parents=True, exist_ok=True)
    mem = Memory(db_path=str(data_dir / "memory" / "researcher.db"), agent_name="researcher")
    agent = AitherAgent(
        name="researcher",
        identity=identity,
        llm=llm,
    ...
    # Zero every limit on the agent's meter so chat() never returns the
    # "monthly token limit" message regardless of license/env state.
    try:
    ...
    # Knowledge graph: reuse the agent's own GraphMemory so save_finding/recall
    # and the agent's auto-ingestion all share one store.
    graph = getattr(agent, "_graph", None)
    if graph is None:
        from adk.graph_memory import GraphMemory
        graph = GraphMemory(agent_name="researcher")

    artifacts = HERE / "artifacts"
    session = ResearchSession(graph=graph, ledger=ledger, artifacts_dir=artifacts,
                              session_id="ds")

    for fn in build_research_tools(session):
        agent._tools.register(fn)
    try:
        register_self_tools(agent)   # honest introspection tools (no hallucinated audits)
    except Exception as exc:  # noqa: BLE001
    ...
    log.info("Tools enabled (%d): %s", len(agent._tools._tools),
             ", ".join(sorted(agent._tools._tools)))

    return agent, ledger, session, bp, provider, model


# ─────────────────────────────────────────────────────────────────────────────
# One research turn (shared by the HTTP route AND the tests — identical logic)
# ─────────────────────────────────────────────────────────────────────────────
async def research_turn(agent, session, message: str, sid: str, on_event=None):
    """Ground the prompt in the agent's OWN memory (RAG), then run adk's reliable
    streaming native-tool loop (stream_chat). Returns (answer, tools_used).

    RAG grounding (not a fallback) is how an agent uses long-term memory: we retrieve
    the most relevant stored facts and put them in front of the model authoritatively,
    with a hard no-fabrication rule, so a long session never forgets and never invents.
    stream_chat then runs native tool-calling (concludes on its own) and STREAMS the
    answer as token events for a live typing effect in the UI.
    """
    grounded = message
    try:
        nodes = await session.graph.search(message, limit=8)
        facts = [getattr(n, "content", "") for n in nodes if getattr(n, "content", "")]
        if facts:
            session.ledger.record_recall("\n".join(facts))  # tokens reused from memory
            block = "\n".join(f"- {f}" for f in facts)[:6000]
            grounded = (
                "Facts from YOUR memory / knowledge graph (authoritative). Answer the "
                "user from these. If a requested detail is NOT present below, say you do "
                "not have it on record — NEVER invent names, numbers, dates, or values.\n"
                f"{block}\n\nUser: {message}"
            )
    except Exception:  # noqa: BLE001
        pass

    resp = await agent.stream_chat(grounded, on_event=on_event, session_id=sid,
                                   token_delay=0.012)
...
# ─────────────────────────────────────────────────────────────────────────────
PROVIDERS = [
    {"id": "anthropic", "label": "Anthropic (Claude)", "key_env": "ANTHROPIC_API_KEY",
     "needs_key": True, "hint": "sk-ant-..."},
    {"id": "openai", "label": "OpenAI", "key_env": "OPENAI_API_KEY",
     "needs_key": True, "hint": "sk-..."},
    {"id": "deepseek", "label": "DeepSeek", "key_env": "DEEPSEEK_API_KEY",
     "needs_key": True, "hint": "sk-..."},
    {"id": "ollama", "label": "Local Ollama (no key)", "key_env": "",
     "needs_key": False, "hint": "runs offline"},
]
_KEY_ENV = {p["id"]: p["key_env"] for p in PROVIDERS}


def create_app():
    bp, _sys_prompt = load_pack()                 # labels available before any key
    state: dict = {"agent": None, "ledger": None, "session": None,
                   "provider": None, "model": None, "error": "not_configured"}

    def rebuild() -> bool:
        try:
            a, lg, se, _bp, prov, mdl = build_agent()
            state.update(agent=a, ledger=lg, session=se, provider=prov, model=mdl, error=None)
            return True
        except SystemExit:
    ...
    app = FastAPI(title="Deep Research Studio")
    artifacts = HERE / "artifacts"
    artifacts.mkdir(parents=True, exist_ok=True)
    app.mount("/files", StaticFiles(directory=str(artifacts)), name="files")
    ...
    @app.get("/health")
    async def health():
        return {"status": "ok", "configured": state["agent"] is not None,
                "provider": state["provider"], "model": state["model"] or "(default)"}

    skills_dir = ASSETS / "pack" / "deep-research" / "skills"
    pack_skills = sorted(p.stem for p in skills_dir.glob("*.md")) if skills_dir.is_dir() else []

    @app.get("/api/config")
    async def config():
        agent = state["agent"]
        # The whitelisted tools (the agent's "hands") — exclude self_* introspection.
        tool_names = sorted(agent._tools._tools) if agent else (bp.get("tools") or [])
        ...
        return {
            ...
            "pack": bp.get("app_name", "Deep Research"),
            "tools": tools,
            "skills": pack_skills,
        }
    ...
    async def settings(request: Request):
        ...
        updates = {"DEEP_RESEARCH_PROVIDER": prov}
        if mdl:
            updates["DEEP_RESEARCH_MODEL"] = mdl
        if _KEY_ENV[prov]:
            updates[_KEY_ENV[prov]] = key
        write_env(updates)
        ok = rebuild()
        return {"ok": ok, "configured": state["agent"] is not None,
                "provider": state["provider"], "error": state["error"]}

    @app.get("/api/ledger")
    async def get_ledger():
        return state["ledger"].snapshot() if state["ledger"] else {
            "used": 0, "saved": 0, "saved_pct": 0.0, "llm_calls": 0,
            "saved_recall": 0, "saved_dedup": 0, "saved_cache": 0,
            "recall_hits": 0, "dedup_hits": 0, "searches": 0, "pages_read": 0, "findings": 0,
        }
    ...
    async def research(request: Request):
        ...
        sid = body.get("session_id") or "ds"
        if not message:
            return JSONResponse({"error": "empty message"}, status_code=400)
        agent, session, ledger = state["agent"], state["session"], state["ledger"]

        async def gen():
            if agent is None:
                yield sse("error", {"error": "Not configured — add your LLM API key in "
                                    "Settings (⚙) to start researching."})
                yield sse("complete", {"ledger": {}})
                return
            ...
            async def run():
                try:
                    content, tools = await research_turn(agent, session, message, sid, on_event)
...
def main():
    parser = argparse.ArgumentParser(description="Deep Research Studio")
